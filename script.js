...
Description: Surprisingly lush slopes...`;
        ...
        function hatchCreature() {
            ...
            loadAndDisplayCreature(creatureData, false, 'hatch'); // MODIFIED: Added sourceAction 'hatch'
        ...
        function createCreatureObject(params) {
            ...
            return {
                ...
                allowActivePanelActions: params.allowActivePanelActions || false // ADDED: Default to false
            };
        }

        // MODIFIED: Added sourceAction parameter
        function loadAndDisplayCreature(creatureInstanceData, isEvolutionDisplayUpdate = false, sourceAction = 'unknown') {
            ...
            // Ensure allowActivePanelActions is part of the creature data being copied/used
            ...
            if (sourceAction === 'hatch') {
                ...
                // If not set by source and not pre-existing, default for safety
                creatureDataWithActionFlag.allowActivePanelActions = false;
            }
            ...
            if (activeCreatureInstance && activeCreatureInstance.userData && activeCreatureInstance.userData.uniqueId === creatureDataWithActionFlag.uniqueId && !isEvolutionDisplayUpdate) {
                 // If re-activating the same creature, ensure its action flag is updated
                activeCreatureInstance.userData.allowActivePanelActions = creatureDataWithActionFlag.allowActivePanelActions;
                updateActiveCreatureDisplay();
                updateButtonState();
                updateStoredCreaturesDisplay();
                return;
            ...
            const modelURL = getModelURL(creatureDataWithActionFlag.modelKey);
            const currentCreatureDataCopy = createCreatureObject({...creatureDataWithActionFlag}); // Work with a fresh copy that includes the flag

            if (modelURL) {
                const loader = new GLTFLoader();
                loader.load(modelURL, (gltf) => {
                    activeCreatureInstance = gltf.scene;
                    activeCreatureInstance.userData = currentCreatureDataCopy; // This now includes allowActivePanelActions
                    ...
                    try {
                        ...
                        activeCreatureInstance.position.set(0,0.5,0); 
                    }
                    ...
                    // MODIFIED: Logic for adding to storage for 'hatch' or 'activation' (isNewlyHatchedOrActivated equivalent)
                    if (sourceAction === 'hatch' || sourceAction === 'activation') {
                        ...
                        if (existingStoredIndex === -1) {
                            ...
                            // If activating, it's already in storage, so update it (e.g. its allowActivePanelActions flag)
        ...
        function updateGameTimers() {
            ...
            if (activeCreatureInstance && activeCreatureInstance.userData) {
                ...
                if (storedVersionIndex > -1) {
                    // Sync back changes, but be careful not to overwrite allowActivePanelActions if it's specific to active instance
                    const { allowActivePanelActions, ...restOfActiveData } = activeData;
                    storedCreatures[storedVersionIndex] = { ...storedCreatures[storedVersionIndex], ...restOfActiveData };
                }
        ...
        function attemptNaturalEvolution() {
            ...
            // MODIFIED: Check if stored and allowed for active panel actions
            const isStored = storedCreatures.some(c => c && c.uniqueId === creatureData.uniqueId);
            if (!isStored || !creatureData.allowActivePanelActions) {
                alert("Creature must be stored and activated to evolve this way.");
                return;
            }
            ...
            if (ev1Def) {
                ...
                loadAndDisplayCreature(creatureData, true, 'evolution'); // MODIFIED: sourceAction
        ...
        function attemptLevelUpCreature() {
            ...
            const isStored = storedCreatures.some(c => c && c.uniqueId === creatureData.uniqueId);
            if (!isStored || !creatureData.allowActivePanelActions) {
                alert("Creature must be stored and activated to level up.");
                return;
            }
        ...
        function triggerEvolutionByLevel(creatureData) {
            if (!creatureData) return;
            // Assumes checks for allowActivePanelActions were done by caller (attemptLevelUpCreature)
            ...
            // allowActivePanelActions remains true
            updateCreatureCanEvolveStatus(creatureData);
            ...
            loadAndDisplayCreature(creatureData, true, 'evolution'); // MODIFIED: sourceAction
            updateStoredCreaturesDisplay(); // Redundant if loadAndDisplayCreature does it
            updateButtonState(); // Redundant
        }
        ...
        function activateStoredCreature(uniqueId) {
            ...
            if (activeCreatureInstance && activeCreatureInstance.userData) {
                ...
                const existingStoredIndex = storedCreatures.findIndex(c => c.uniqueId === currentActiveData.uniqueId);
                 // Update stored version of previously active creature, but preserve its allowActivePanelActions if it was set
                if (existingStoredIndex !== -1) {
                    storedCreatures[existingStoredIndex] = { ...currentActiveData };
                } else if (storedCreatures.length < MAX_STORED_CREATURES) {
                     // This case should ideally not happen if active creature that's not stored is handled correctly.
                    storedCreatures.push({...currentActiveData});
                }
            }
            
            // MODIFIED: When activating, set allowActivePanelActions to true for the instance being loaded
            const creatureDataForActivation = { ...creatureToActivate, allowActivePanelActions: true };
            selectedForMating = []; // Clear mating selection
            loadAndDisplayCreature(creatureDataForActivation, false, 'activation'); // MODIFIED sourceAction
            // loadAndDisplayCreature will call updateStoredCreaturesDisplay, which will hide action buttons due to empty selectedForMating
        }
        ...
        function attemptStoreActiveCreature() {
            ...
            const creatureDataToStore = {...activeCreatureInstance.userData}; // Make a copy
             // When storing, actions from active panel are no longer relevant for *this specific active instance*.
            // The stored copy will get its allowActivePanelActions reset to false by default if re-activated freshly.
            // Or, more simply, allowActivePanelActions is a property of the *active* instance, not inherently of the stored data.
            // When activated, it's set to true. When hatched, to false.
            // So, the creatureDataToStore already has its flag from when it became active.
            // If it was a hatched creature (flag=false), storing it doesn't change that for the stored copy.
            // If it was an activated creature (flag=true), storing it keeps that for the stored copy (which is fine).
        ...
        // ADDED: Function to remove creature from storage
        function removeStoredCreature(uniqueId) {
            const creatureData = storedCreatures.find(c => c && c.uniqueId === uniqueId);
            if (!creatureData) return;

            // Optional: Confirmation
            // if (!confirm(`Are you sure you want to remove ${creatureData.name || 'this creature'}? This action cannot be undone.`)) {
            //     return;
            // }
            ...
            updateStoredCreaturesDisplay(); // Redraws list, hides all slot actions by default
                                        // and correctly applies 'selected' based on new selectedForMating
            updateButtonState();

            // Explicitly re-evaluate button visibility for any remaining single selection
            if (selectedForMating.length === 1) {
                const itemLi = storedCreaturesList.querySelector(`li[data-creature-unique-id="${selectedForMating[0]}"]`);
                if (itemLi) {
        ...
        function setupMating() {
            ...
            if (!areCreaturesCompatible(parent1ForMating, parent2ForMating)) { alert("Creatures not compatible."); selectedForMating = []; updateStoredCreaturesDisplay(); updateButtonState(); return; }
        ...
        function updateStoredCreaturesDisplay() {
            ...
            for (let i = 0; i < MAX_STORED_CREATURES; i++) {
                ...
                if (creature && creature.uniqueId !== undefined && creature.color instanceof THREE.Color) {
                    updateCreatureCanEvolveStatus(creature);
                    li.dataset.creatureUniqueId = creature.uniqueId; // Store uniqueId on the <li>
                    ...
                    if (selectedForMating.includes(creature.uniqueId)) { // Apply 'selected' class based on current mating selection
                    ...
                    // MODIFIED: Create wrapper for action buttons, initially hidden
                    const actionsWrapper = document.createElement('div');
                    actionsWrapper.classList.add('slot-actions');
                    actionsWrapper.style.display = 'none'; // Hidden by default
                    ...
                    activateButton.onclick = (e) => {
                        e.stopPropagation(); // Prevent li click from firing toggleMatingSelection again
                        activateStoredCreature(creature.uniqueId);
                    };
                    ...
                    removeButton.onclick = (e) => {
                        e.stopPropagation();
                        removeStoredCreature(creature.uniqueId);
                    };
                    actionsWrapper.appendChild(removeButton);
                    li.appendChild(actionsWrapper);


                    // MODIFIED: Click on infoWrapper (or li) toggles mating selection AND button visibility
                    infoWrapper.style.cursor = 'pointer';
                    infoWrapper.onclick = () => toggleMatingSelection(creature.uniqueId);
                    // Or if you prefer the whole li:
                    // li.style.cursor = 'pointer';
                    // li.onclick = () => toggleMatingSelection(creature.uniqueId);
            ...
            // After rebuilding the list, ensure correct buttons are shown based on current selectedForMating
            if (selectedForMating.length === 1) {
                const onlySelectedLi = storedCreaturesList.querySelector(`li[data-creature-unique-id="${selectedForMating[0]}"] .slot-actions`);
                if (onlySelectedLi) onlySelectedLi.style.display = 'flex'; // Or 'block' or your chosen display for slot-actions
            }
        }


        // MODIFIED: toggleMatingSelection now also handles button visibility
        function toggleMatingSelection(creatureUniqueId) {
            ...
            if (!creature || creature.currentEvolutionStage !== 0) {
                ...
                // Clear selection if invalid creature was part of it
                const invalidSelectionIndex = selectedForMating.findIndex(id => {
                    const c = storedCreatures.find(stored_c => stored_c && stored_c.uniqueId === id);
                    return !c || c.currentEvolutionStage !== 0;
                });
                if (invalidSelectionIndex > -1) selectedForMating = [];

            } else if (activeCreatureInstance && activeCreatureInstance.userData && activeCreatureInstance.userData.uniqueId === creatureUniqueId) {
                alert("Cannot select active creature for mating.");
                return;
            }
            ...
            if (index > -1) { // Creature is already selected, deselect it
                ...
                if (creature.currentEvolutionStage === 0) { // Only allow selection if it's a base creature
                    if (selectedForMating.length < 2) {
                        selectedForMating.push(creatureUniqueId);
                    } else { // Already 2 selected, replace the first one
                        selectedForMating.shift();
                        selectedForMating.push(creatureUniqueId);
                    }
                } else {
                    // If trying to select a non-base creature when others are selected for mating,
                    // it implies an intent change, so clear existing mating selection.
                    selectedForMating = [];
                }
            }

            // Update button visibility and 'selected' class for all storage slots
            const creatureItems = storedCreaturesList.querySelectorAll('.stored-creature-item');
            creatureItems.forEach(itemLi => {
                const itemUniqueIdStr = itemLi.dataset.creatureUniqueId;
                if (!itemUniqueIdStr) return; // Skip empty slots or those without ID
                const itemUniqueId = parseInt(itemUniqueIdStr);

                const actionsDiv = itemLi.querySelector('.slot-actions');
                const creatureInSlot = storedCreatures.find(c => c && c.uniqueId === itemUniqueId);

                if (actionsDiv && creatureInSlot) { // Check if creature slot is not empty and div exists
                    // Show buttons only if one creature is selected AND it's this one AND it's not for mating (stage 0)
                    if (selectedForMating.length === 1 && selectedForMating[0] === itemUniqueId) {
                        actionsDiv.style.display = 'flex'; // Or 'block' or your chosen display for .slot-actions
                    } else {
        ...
        function updateButtonState() {
            ...
            let creatureIsStoredAndPermitted = false; // MODIFIED: New condition
            if (activeCreatureData && activeCreatureData.uniqueId !== undefined) {
                ...
                // MODIFIED: Check allowActivePanelActions flag
                if (isStored && activeCreatureData.allowActivePanelActions) {
                    creatureIsStoredAndPermitted = true;
                }
            }
            ...
            if (dynamicLevelUpButton) {
                ...
                // MODIFIED: Use creatureIsStoredAndPermitted
                if (activeCreatureData && !isIncubating && creatureIsStoredAndPermitted) {
            ...
            if(isIncubating && typeof updateStoredCreaturesDisplay === 'function') {
                // No direct call to updateStoredCreaturesDisplay needed here unless specifically for button states within items during incubation,
                // but toggleMatingSelection and initial draw handle visibility.
                // Activating buttons within stored items should be disabled during incubation anyway by their own logic.
            }
        }
       ...
       function setupHybridRules() {
            ...
            const interHybridSetup = [ /* ... content as before ... */ ]; 
